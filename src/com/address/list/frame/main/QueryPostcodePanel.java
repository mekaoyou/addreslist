...
public class QueryPostcodePanel extends JPanel
{
	...
	public QueryPostcodePanel(UserFrame user,String username)
	{
		this.user=user;
	...
	public void init()
	{
		...
		this.setBorder(BorderFactory.createTitledBorder(BorderFactory.createRaisedBevelBorder(),"     ------  查询邮编  ------"));
		//顶部panel
		...
		selectButton.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				if(code.length()!=6)
				{
					new FlyDialog(user.getUserFrame(), "请输入正确的邮编");
					return;
				}
				MobileEntity obj = MobileDao.getInstance().getPostByCode(code);
				if(obj == null)
				{
					new FlyDialog(user.getUserFrame(), "邮编不存在");
					return;
				}

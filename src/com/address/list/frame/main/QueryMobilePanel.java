...
/**
 * 自定义组件，用户实现查询手机号码归属地的功能
 * @author Alex
 *
 */
@SuppressWarnings("serial")
public class QueryMobilePanel extends JPanel
{
	...
	public QueryMobilePanel(UserFrame user,String username)
	{
	...
	public void init()
	{
		...
		this.setBorder(BorderFactory.createTitledBorder(BorderFactory.createRaisedBevelBorder(),"     ------  查询手机号码归属地  ------"));
		//顶部panel
		...
		selectButton.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				{
					new FlyDialog(user.getUserFrame(), "请输入正确的手机号码");
					return;
				}
				MobileEntity obj = MobileDao.getInstance().getPostByMoble(code.substring(0, 7));
				if(obj == null)
				{
					new FlyDialog(user.getUserFrame(), "手机号码不存在");
					return;
				}
				result.setText(obj.getArea() + "\t" + obj.getMobileType());
			}
		});
